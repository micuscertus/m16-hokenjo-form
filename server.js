...
// 使うことがそもそも想定されない飲み物用の選択肢のため、生食材の加熱要否判定の対象外でよい
...
最重要ルール２（他のどの指示より優先します）：コーヒー豆・コーヒー粉に関する記載（調理方法の材料欄が「豆」か「粉」か、仕込み内容欄や調理器具欄・調理の動作欄に焙煎・挽くといった言葉があるか）について、一切判断・指摘しないでください。この判断は別の仕組みが既に行っています。
...
- 調理の動作欄・その他の動作欄での「茹でる」（大量の水を使う調理）の指摘（仕込み内容欄は許可施設で行う前提のため対象外。仕込み内容欄に「茹でる」とあっても指摘不要）
- 購入先の空欄・海外住所
...
- コーヒー・紅茶等をまとめて作り置きする点の指摘（市販のティーバッグ・コーヒー粉を使って一杯ずつ抽出する形への変更）
- コーヒーのドリッパーが「使い捨て」でも「カセット式」でもない点の指摘
- コーヒーの材料欄に「豆」とだけ、または「珈琲豆」「コーヒー豆」と書かれている点の指摘（「コーヒー粉」への変更）
...
- 調理方法の②調理器具（鉄板／フライパン／鍋／フライヤー／蒸し器／直火等、その他の自由記述含む）が取扱食品名から見て一般的・伝統的かどうかの判断・指摘（例：「焼き鳥」に「鉄板」は不自然で「七輪」「焼き鳥用コンロ」等にすべき、といった一般常識との照合はAIチェックの対象外です。出店者が実際に使う調理器具を選んでいるだけなので、食品名との組み合わせの妥当性は絶対に指摘しないでください）
...
必ず以下のJSON形式のみで返答してください:
{
  ...
  // コーヒー関連の提出物は、仕込み内容欄・調理方法の①②③その他自由記述欄の豆・焙煎関連ワードをAIに見せる前に除去する
