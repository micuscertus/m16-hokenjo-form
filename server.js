...
入力されたJSONの内容を見て、食品衛生上の危険や、記載内容が明らかに矛盾していて保健所で確認が必要になる点だけを指摘してください（ただし上記の最重要ルールの範囲は除く）。
...
- 食品名や食材から「この食品は温かい状態で提供されるものだろう」と推測し、保存方法（常温・冷蔵・冷凍・その他のいずれか）の選択が提供時の温度と食い違っている、といった指摘（このフォームの保存方法欄は常温・冷蔵・冷凍・その他の4択のみで、「温かい状態で保温する」のような選択肢はそもそも存在しません。出店者が選べない選択肢への変更を求める指摘は絶対にしないでください）
...
- コーヒー豆を当日・会場でその場で挽く／粉にする点の判断・指摘（機械チェックで別途対応済みのため対象外。材料欄・調理方法欄等に「コーヒー粉」「豆を挽く」のような曖昧でない記載があっても、焙煎工程や粉砕のタイミングまで具体的に明記するよう求める必要はありません。「豆」をその場に持ち込んで挽く旨が明記されている場合のみ、機械チェック側で判定するので、AIチェックでは一切判断しないこと）
- 材料欄が「コーヒー粉」で、仕込み内容欄・調理方法欄に「豆を焙煎する」等の焙煎・仕入れ工程の記載がある組み合わせを、材料と仕込み内容の食い違いとして指摘すること（コーヒー粉を最終的に会場へ持ち込む前提であれば、その手前の工程で豆の状態を経由すること自体は矛盾ではありません。材料欄を「コーヒー豆」に書き換えるよう求める指摘も禁止です）
